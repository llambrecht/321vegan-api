...
class FileService:
    ...
    def save_partner_logo(self, partner_id: int, file: UploadFile) -> str:
        """
        Save the partner logo and return the relative path.
        ...
        file_extension = Path(file.filename or "").suffix.lower()
        filename = f"partner_{partner_id}_{uuid.uuid4().hex}{file_extension}"
        file_path = self.partners_dir / filename
        
        try:
            self.delete_old_partner_logo(partner_id)
    ...
    def delete_old_partner_logo(self, partner_id: int) -> bool:
        """Delete the old logo of a partner."""
        try:
            pattern = f"partner_{partner_id}_*"
            for file_path in self.partners_dir.glob(pattern):
                file_path.unlink()
            return True
        except Exception:
            return False
    
    def delete_partner_logo(self, logo_path: str) -> bool:
        """Delete a partner logo by path."""
        try:
            full_path = Path(logo_path)
            if full_path.exists() and full_path.is_relative_to(self.upload_dir):
                full_path.unlink()
