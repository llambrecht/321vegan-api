...
class ProductFoundReportCRUDRepository(CRUDRepository):
    def has_reported_today(self, db: Session, ean: str, shop_id: int, user_id: int) -> bool:
        """
        Check if a user already reported this product as found at this shop today.
    ...
    def get_reports_for_shop(self, db: Session, shop_id: int) -> List[ProductFoundReport]:
        """
        Get all found reports for a shop.
        ...
        Returns:
            List[ProductFoundReport]: All reports for the shop.
        """
        return db.query(self._model).filter(
            self._model.shop_id == shop_id
        ).all()
    ...
    def get_reports_for_shop_ean(
    ...
    ) -> List[ProductFoundReport]:
        """
        Get found reports for a specific product at a shop, optionally after a date.
        ...
        Returns:
            List[ProductFoundReport]: Matching reports.
        """
...
product_found_report_crud = ProductFoundReportCRUDRepository(model=ProductFoundReport)
